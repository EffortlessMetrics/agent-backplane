...
//! Microcrate for wiring the OpenAI Chat Completions sidecar into ABP runtimes.
//!
//! Registers the OpenAI sidecar backend and exposes the
...
pub const BACKEND_NAME: &str = "sidecar:openai";

/// Relative path to the JS OpenAI host inside a workspace checkout.
pub const HOST_SCRIPT_RELATIVE: &str = "hosts/openai/host.js";

/// Preferred executable for the default OpenAI sidecar host.
pub const DEFAULT_NODE_COMMAND: &str = "node";

/// Register the OpenAI sidecar backend if available.
...
/// Register an OpenAI backend under a custom name.
pub fn register_backend(
    ...
    let command = resolve_command(command_override)
        .with_context(|| format!("resolve OpenAI command for {backend_name}"))?;
...
fn resolve_command(command_override: Option<&str>) -> Result<Option<String>> {
    if let Some(command) = command_override {
        ...
        if !command.is_empty() {
            ...
            anyhow::bail!("explicit OpenAI command '{command}' is not available");
        }
    }
...
mod tests {
    ...
    fn backend_name_is_correct() {
        assert_eq!(BACKEND_NAME, "sidecar:openai");
    }
