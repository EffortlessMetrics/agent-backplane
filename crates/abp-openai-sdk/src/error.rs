...
//!
//! Models the error envelope returned by the OpenAI REST API so that
//! ABP shims can produce wire-compatible error responses.
...
/// Top-level error response returned by the OpenAI API.
///
/// ```json
...
mod tests {
    ...
    fn error_response_matches_openai_json_format() {
        ...
        let json = serde_json::to_string(&resp).unwrap();
        // Must have the top-level "error" key
    ...
    #[test]
    fn deserialize_from_openai_json() {
        let json = r#"{
            "error": {
    ...
    #[test]
    fn deserialize_invalid_request_with_param() {
        let json = r#"{
            "error": {
                "message": "'gpt-5' is not a valid model.",
        ...
        let resp: ErrorResponse = serde_json::from_str(json).unwrap();
        assert_eq!(resp.error.param.as_deref(), Some("model"));
    ...
    fn null_param_and_code_serialize_correctly() {
        ...
        let json = serde_json::to_string(&err).unwrap();
        // OpenAI sends null for these fields, not omitting them
