...
//! Strongly-typed Kimi (Moonshot) Chat Completions API types.
//!
//! Kimi uses an OpenAI-compatible API with extensions for web search.
...
// Kimi shares most of the OpenAI wire format; we define Kimi-specific
// wrappers that add the search extension fields.
...
pub struct SearchOptions {
    ...
    pub mode: SearchMode,
    /// Maximum number of search results to incorporate.
...
pub struct KimiChatRequest {
    ...
    /// Fine-grained web search configuration.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct KimiChatResponse {
    ...
    /// Token usage statistics (with Kimi extensions).
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct KimiStreamChunk {
    ...
    /// Usage info (Kimi may include this on the final chunk).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub usage: Option<KimiUsage>,
}
...
pub struct KimiUsage {
    ...
    /// Tokens consumed by web search results (Kimi extension).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub search_tokens: Option<u32>,
}
