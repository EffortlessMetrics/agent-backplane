...
//!
//! Provides [`KimiConfig`] for vendor-specific configuration and mapping
//! functions ([`map_work_order`], [`map_response`]) for translating between
//! ABP contract types and the Moonshot/Kimi chat completions API format.
#![deny(unsafe_code)]
...
pub fn register_backend(
    ...
    let command = resolve_command(command_override)
        .with_context(|| format!("resolve Kimi command for {backend_name}"))?;
...
fn resolve_command(command_override: Option<&str>) -> Result<Option<String>> {
    if let Some(command) = command_override {
        ...
        if !command.is_empty() {
            ...
            anyhow::bail!("explicit Kimi command '{command}' is not available");
        }
    }
