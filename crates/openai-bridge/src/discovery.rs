...
/// 2. ABP_OPENAI_HOST_SCRIPT environment variable
/// 3. hosts/openai/host.js relative to CWD
/// 4. ~/.agent-backplane/hosts/openai/host.js
pub fn resolve_host_script(explicit: Option<&Path>) -> Result<PathBuf, BridgeError> {
    ...
    Err(BridgeError::HostScriptNotFound(
        "could not find hosts/openai/host.js in any search path".to_string(),
    ))
}
