...
///
/// ```no_run
/// # use abp_workspace_quota::WorkspaceQuota;
...
mod tests {
    ...
    #[test]
    fn cleanup_ignores_git_directory() {
        let dir = tempdir().expect("tempdir");
        write_file(dir.path(), "payload.txt", &"x".repeat(512));
        write_file(dir.path(), ".git/objects/secret", &"y".repeat(2048));

        let q = WorkspaceQuota::new(400);
        let result = q.cleanup(dir.path()).expect("cleanup result");
        assert!(result.files_deleted > 0);
        assert!(dir.path().join(".git/objects/secret").exists());
    }
}
