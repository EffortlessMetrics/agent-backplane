...
//! Copilot-specific error types for the shim layer.
//!
//! These error types mirror the GitHub Copilot API error responses so that
//! downstream consumers see the same error taxonomy they would get from
//! the official Copilot Extensions API.
...
/// An API error returned by the Copilot-compatible endpoint.
///
/// Wraps a status code and an [`ErrorBody`] that matches the Copilot
/// `{"error": {...}}` envelope.
...
/// The `"error"` object inside a Copilot API error response.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema)]
...
// ── Convenience constructors for common Copilot errors ──────────────────
...
mod tests {
    ...
    fn authentication_error_construction() {
        let err = AuthenticationError::new("Invalid Copilot token");
        assert_eq!(err.inner.status, 401);
        assert_eq!(err.inner.body.error_type, "authentication_error");
        assert_eq!(err.inner.body.code.as_deref(), Some("invalid_token"));
    }
    ...
    fn api_error_is_auth() {
        let err = AuthenticationError::new("bad token").inner;
        assert!(err.is_auth_error());
        assert!(!err.is_rate_limit());
        assert!(!err.is_retryable());
    }
    ...
    #[test]
    fn error_body_serde_matches_copilot_format() {
