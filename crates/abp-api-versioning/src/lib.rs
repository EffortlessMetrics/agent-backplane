...
#![deny(unsafe_code)]
#![warn(missing_docs)]
//! API version parsing, registry, and negotiation primitives.
...
mod tests {
    ...
    #[test]
    fn parses_prefixed_and_unprefixed_versions() {
        assert_eq!(
            ApiVersion::parse("v1.2").unwrap(),
            ApiVersion { major: 1, minor: 2 }
        );
        assert_eq!(
            ApiVersion::parse("3").unwrap(),
            ApiVersion { major: 3, minor: 0 }
        );
    }

    #[test]
    fn negotiates_highest_compatible_not_newer_than_request() {
        let supported = [
            ApiVersion { major: 1, minor: 0 },
            ApiVersion { major: 1, minor: 3 },
            ApiVersion { major: 2, minor: 0 },
        ];
        let requested = ApiVersion { major: 1, minor: 2 };
        assert_eq!(
            VersionNegotiator::negotiate(&requested, &supported),
