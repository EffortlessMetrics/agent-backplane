...
//!
//! Provides [`CodexConfig`] for vendor-specific configuration and mapping
//! functions ([`map_work_order`], [`map_response`]) for translating between
//! ABP contract types and the OpenAI Codex/Responses API format.
#![deny(unsafe_code)]
...
fn resolve_command(command_override: Option<&str>) -> Result<Option<String>> {
    if let Some(command) = command_override {
        ...
        if !command.is_empty() {
            ...
            anyhow::bail!("explicit Codex command '{command}' is not available");
        }
    }
